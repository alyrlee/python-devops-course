#!/usr/bin/env python3
"""
Simplified AWS Lambda Deployment Script
Uses existing IAM role or creates function without role creation
"""
...
class SimpleLambdaDeployer:
    """Simplified AWS Lambda deployment manager"""
...
"""
    ...
    def find_existing_role(self):
        """Try to find an existing Lambda execution role"""
        try:
            iam_client = boto3.client('iam')
            
            # Try to find common Lambda roles
            common_roles = [
                'Lambda_Service',  # Found in your account
            ...
            for role_name in common_roles:
                try:
                    ...
                    click.echo(click.style(f"✓ Found existing role: {role_name}", fg="green"))
                    return role_arn
                except ClientError:
                    continue
            
            # If no common roles found, try to list roles and find one with Lambda permissions
            try:
                response = iam_client.list_roles(MaxItems=50)
                for role in response['Roles']:
                    if 'lambda' in role['RoleName'].lower() or 'execution' in role['RoleName'].lower():
                        click.echo(click.style(f"✓ Found potential role: {role['RoleName']}", fg="yellow"))
                        return role['Arn']
            except ClientError:
                pass
            ...
            click.echo(click.style(f"⚠ Could not search for existing roles: {str(e)}", fg="yellow"))
            return None
    
    def deploy_lambda_function(self, function_name, zip_path, role_arn=None):
        """Deploy Lambda function with existing role or default"""
        try:
            # Read the zip file
            with open(zip_path, 'rb') as f:
                zip_content = f.read()
            
            # If no role provided, try to find one
            if not role_arn:
                role_arn = self.find_existing_role()
            
            # If still no role, use a basic execution role ARN
            if not role_arn:
                # Get account ID
                sts_client = boto3.client('sts')
                account_id = sts_client.get_caller_identity()['Account']
                role_arn = f"arn:aws:iam::{account_id}:role/lambda-execution-role"
                click.echo(click.style(f"⚠ Using default role ARN: {role_arn}", fg="yellow"))
                click.echo(click.style("Note: You may need to create this role manually in AWS Console", fg="yellow"))
            ...
            try:
                ...
                try:
                    ...
                    if 'InvalidParameterValueException' in str(create_error):
                        click.echo(click.style("✗ Role ARN is invalid. Please create a Lambda execution role manually:", fg="red"))
                        click.echo(click.style("1. Go to AWS IAM Console", fg="blue"))
                        click.echo(click.style("2. Create role with 'Lambda' service", fg="blue"))
                        click.echo(click.style("3. Attach 'AWSLambdaBasicExecutionRole' policy", fg="blue"))
                        click.echo(click.style("4. Use the role ARN in deployment", fg="blue"))
                        return None
                    else:
                        raise
...
@click.option('--role-arn', help='Existing IAM role ARN for Lambda')
def main(function_name, region, role_arn):
    """Deploy Lambda function to AWS"""
    
    click.echo(click.style("🚀 Starting simplified Lambda deployment...", fg="blue", bold=True))
    
    # Initialize deployer
    deployer = SimpleLambdaDeployer(region)
