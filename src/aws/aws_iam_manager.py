...
class AWSIAMManager:
    ...
    def __init__(self, region: str = 'us-east-1'):
        ...
        try:
            ...
            self.region = region
            click.echo(click.style("✓ AWS IAM Manager initialized successfully", fg="green"))
    ...
    def create_cloud9_role(self, role_name: str) -> bool:
        ...
        try:
            ...
            click.echo(click.style(f"✓ Created Cloud9 service role: {role_name}", fg="green"))
            return True
        ...
        try:
            for policy_arn in policy_arns:
                try:
                    ...
                    click.echo(click.style(f"✓ Attached policy: {policy_name}", fg="green"))
                except ClientError as e:
                    if e.response['Error']['Code'] == 'EntityAlreadyExists':
                        policy_name = policy_arn.split('/')[-1]
                        click.echo(click.style(f"⚠ Policy {policy_name} already attached", fg="yellow"))
                    else:
    ...
    def attach_role_to_user(self, username: str, role_name: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                click.echo(click.style(f"✓ Policy {policy_name} already attached to user", fg="yellow"))
                return True
            except ClientError as e:
                if e.response['Error']['Code'] != 'NoSuchEntity':
                    raise
            ...
            click.echo(click.style(f"✓ Attached role {role_name} to user {username}", fg="green"))
            return True
            
        except ClientError as e:
            click.echo(click.style(f"✗ Error attaching role to user: {str(e)}", fg="red"))
            return False
    ...
    def create_access_key(self, username: str) -> Optional[dict]:
        ...
        try:
            ...
            click.echo(click.style(f"✓ Created access key for user {username}", fg="green"))
            return access_key
        except ClientError as e:
...
@click.command()
@click.option('--role-name', default='Cloud9ServiceRole', help='Name of the Cloud9 service role')
@click.option('--username', default='cloud9-user', help='Name of the IAM user')
@click.option('--region', default='us-east-1', help='AWS region')
@click.option('--create-access-key', is_flag=True, help='Create access key for the user')
def main(role_name: str, username: str, region: str, create_access_key: bool):
    ...
    click.echo(click.style("🚀 Starting AWS IAM setup for Cloud9...", fg="blue", bold=True))
    ...
    if iam_manager.setup_cloud9_environment(role_name, username):
        click.echo(click.style("✅ Cloud9 environment setup completed successfully!", fg="green", bold=True))
        ...
        if create_access_key:
            ...
            if access_key:
                ...
                click.echo(click.style("\n⚠️  IMPORTANT: Save these credentials securely!", fg="red", bold=True))
    else:
        click.echo(click.style("❌ Cloud9 environment setup failed!", fg="red", bold=True))
        sys.exit(1)
