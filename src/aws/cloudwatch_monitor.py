...
class CloudWatchMonitor:
    ...
    def __init__(self, region: str = 'us-east-1'):
        ...
        try:
            ...
            click.echo(click.style(f"✗ Error initializing CloudWatch Monitor: {str(e)}", fg="red"))
            exit(1)
    ...
    def get_lambda_metrics(self, function_name: str, hours: int = 24) -> Dict:
        ...
        try:
            ...
            metrics['invocations'] = sum(point['Sum'] for point in invocations['Datapoints'])
            metrics['errors'] = sum(point['Sum'] for point in errors['Datapoints'])
            metrics['avg_duration'] = sum(point['Average'] for point in duration['Datapoints']) / len(duration['Datapoints']) if duration['Datapoints'] else 0
            metrics['max_duration'] = max(point['Maximum'] for point in duration['Datapoints']) if duration['Datapoints'] else 0
    ...
    def create_custom_metric(self, namespace: str, metric_name: str, value: float, unit: str = 'Count'):
        ...
        try:
            ...
            click.echo(click.style(f"✓ Custom metric {metric_name} created", fg="green"))
            return True
        except ClientError as e:
    ...
    def get_lambda_function_info(self, function_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            click.echo(click.style(f"✗ Error getting function info: {str(e)}", fg="red"))
            return None
...
def main(function_name: str, region: str, hours: int, logs: bool, metrics: bool, info: bool):
